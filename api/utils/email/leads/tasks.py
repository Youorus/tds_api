...
def send_appointment_confirmation_task(lead_id: int):
    ...
    if lead and lead.email:
        ...
        logger.warning(f"❌ Aucune confirmation envoyée (lead #{lead_id} inexistant ou sans email)")
...
def send_appointment_planned_task(lead_id: int):
    ...
    if lead and lead.email:
        ...
        logger.warning(f"❌ RDV planifié non envoyé (lead #{lead_id} inexistant ou sans email)")
...
def send_dossier_status_notification_task(lead_id: int):
    ...
    if lead and lead.statut_dossier:
        ...
        logger.info(f"📨 Statut dossier '{lead.statut_dossier.label}' envoyé pour lead #{lead.id}")
    else:
        logger.warning(f"❌ Notification de statut dossier non envoyée (lead #{lead_id} ou statut manquant)")
