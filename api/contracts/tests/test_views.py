...
@pytest.fixture
def admin_user():
    return User.objects.create_user(email="admin@tds.fr", password="test", role="ADMIN", first_name="Admin", last_name="Test")
...
def contract(client_with_lead, admin_user):
    service = Service.objects.create(code="TEST_SERVICE", label="Test Service", price=Decimal("150.00"))
...
def test_refund_contract(auth_client, contract):
    # Simule des paiements
    contract.refund_amount = Decimal("0.00")
    PaymentReceipt.objects.create(contract=contract, client=contract.client, amount=Decimal("120.00"))
    contract.save()

    url = reverse("contract-refund", kwargs={"pk": contract.id})
    payload = {
        "refund_amount": "50.00",
        "refund_note": "Client annulé"
    }
