...
class TestServiceViewSet:
    ...
    def test_update_service_as_non_admin(self, api_client, conseiller_user, service_sample):
        api_client.force_authenticate(user=conseiller_user)
        url = reverse("services-detail", args=[service_sample.id])
    ...
    def test_delete_service_as_non_admin(self, api_client, conseiller_user, service_sample):
        api_client.force_authenticate(user=conseiller_user)
        url = reverse("services-detail", args=[service_sample.id])
        response = api_client.delete(url)
        assert response.status_code == status.HTTP_403_FORBIDDEN