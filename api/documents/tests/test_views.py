...
def user(db):
    return User.objects.create_user(
        email="test@user.com", first_name="Marc", last_name="Nkue",
...
class TestDocumentAPI:
    def test_upload_single_document(self, client, user, monkeypatch):
        ...
        monkeypatch.setattr("api.documents.views.store_client_document", lambda client, file, name: "https://cloud.test/mock.pdf")
        from io import BytesIO
        file = BytesIO(b"dummy content")
        file.name = "file.pdf"
        resp = api_client.post(url + f"?client={client.id}", {"file": file}, format="multipart")
    ...
    def test_upload_multi_files(self, client, user, monkeypatch):
        ...
        file2.name = "doc2.pdf"
        resp = api_client.post(url + f"?client={client.id}", {"files": [file1, file2]}, format="multipart")
        assert resp.status_code == 201
    ...
    def test_list_documents(self, client, user):
        ...
        Document.objects.create(client=client, url="https://cloud.test/doc1.pdf")
        Document.objects.create(client=client, url="https://cloud.test/doc2.pdf")
    ...
    def test_delete_document(self, client, user, monkeypatch):
        api_client = APIClient()
        api_client.force_authenticate(user=user)
        doc = Document.objects.create(client=client, url="https://cloud.test/todelete.pdf")
        url = reverse("document-detail", args=[doc.id])
        monkeypatch.setattr("api.storage_backends.MinioDocumentStorage.delete", lambda self, path: None)
    ...
    def test_permission_required(self, client):
        api_client = APIClient()
        doc = Document.objects.create(client=client, url="https://cloud.test/protected.pdf")
        url = reverse("document-detail", args=[doc.id])
        resp = api_client.delete(url)
