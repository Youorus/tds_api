...
from api.clients.enums import Civilite, SituationFamiliale, SituationProfessionnelle, LogementType, VisaType
...
class Client(models.Model):
    ...
    source = JSONField(
        blank=True,
        default=list,
        verbose_name=_("source d'information")
    )
    ...
    pays = models.CharField(
        _('pays'), max_length=100, blank=True
    )
    nationalite = models.CharField(
        _('nationalité'), max_length=100, blank=True
    )

    # Adresse
    adresse = models.CharField(
        _('adresse'), max_length=255, blank=True
    )
    code_postal = models.CharField(
        _('code postal'), max_length=20, blank=True
    )
    ville = models.CharField(
        _('ville'), max_length=100, blank=True
    )
    ...
    # Demande
    type_demande = models.ForeignKey("services.Service", on_delete=models.PROTECT, null=True, blank=True)
    ...
    # Visa
    a_un_visa = models.BooleanField(
        _('visa'), null=True, blank=True
    )
    ...
    a_des_enfants = models.BooleanField(
        _('a des enfants ?'), null=True, blank=True
    )
    ...
    a_deja_eu_oqtf = models.BooleanField(
        _('déjà eu une OQTF ?'), null=True, blank=True
    )
    ...
    remarques = models.TextField(
        _('remarques'), blank=True
    )

    # Timestamps
    created_at = models.DateTimeField(
        _('créé le'), auto_now_add=True
    )
    updated_at = models.DateTimeField(
        _('modifié le'), auto_now=True
    )
