...
def test_lead_str_affichage():
    ...
    lead = Lead.objects.create(first_name="Alice", last_name="Durand", phone="+33612345678", status=status)
...
def test_statut_defaut_est_defini_si_absent():
    ...
    lead = Lead.objects.create(first_name="Bob", last_name="Martin", phone="+33600000000", status=statut_defaut)
...
def test_statut_reste_confirme_si_rdv_present():
    ...
    rdv = timezone.now()
    lead = Lead.objects.create(first_name="Chloé", last_name="Bernard", phone="+33611111111", appointment_date=rdv, status=statut_rdv_confirme)
